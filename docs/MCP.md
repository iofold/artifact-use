...
- `artifact_comments`: list (`status` open|sent|resolved|all, `since`, `wait` up to 25 s to long-poll for new feedback; every result carries `next_since` to pass back as `since`), reply to, resolve, or reopen comment threads; `subscribe` a webhook (`url`, optional `events`, optional `artifact`; the signing `secret` is returned once), `unsubscribe` (`webhook_id`), and `webhooks` to list them. Comments carry `author_kind` / `agent_label` (creator-token and delegated-agent writes are `agent`) and a `target` with element context (`tag`, `caption`, `src`, `heading`, `index`, `viewport`). `status: "sent"` is the queue of threads viewers explicitly sent to the agent; act on it first.
